...
智能吉他和弦教学系统 — 通过摄像头+麦克风实时检测吉他指法并给出教学反馈。组合了 YOLO（后端检测琴枕/琴桥）、MediaPipe（前端手部关键点）、音高检测（YIN 算法）和 LLM（DeepSeek 主力 + 火山方舟备用）生成教学建议。
...
- 启动时会检查 `models/best.pt` 是否存在，缺失则退出
...
| 模块 | 职责 |
|------|------|
| `app.py` | Flask 入口 + Socket.IO 事件处理（`hand_landmarks`, `thumbnail`, `frame` 等） |
...
| `core/llm_service.py` | DeepSeek 主力 + 火山方舟备用 LLM（对话管理、教学建议生成） |
...
- 缩略图宽度 `THUMBNAIL_WIDTH = 640` 前后端必须同步
- `FORCE_NUT_LEFT` 控制琴枕方向（镜像问题），按需切换
- `models/best.pt` 是训练产物用于 YOLO 检测
